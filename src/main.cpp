...
#if defined(NDEBUG)
# error "LitecoinX cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xa8eca5eed50a912e0899466368e46e97e663806ab8c5fddecb4113812d9aaf20");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // LitecoinX: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "LitecoinX Signed Message:\n";
...
{
    // LitecoinX: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 45; // LitecoinX: 45 Second Retarget
static const int64 nTargetSpacing = 45; // LitecoinX: 45 Second Block Target
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // LitecoinX: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // LitecoinX: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "In LitecoinX we trust!";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0xee6079b97416f29c26419a13ebd73481577c00e47060d059ec40d10fb21b7afc"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // LitecoinX: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("LitecoinXMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("LitecoinXMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static LitecoinXMiner(CWallet *pwallet)
{
    printf("LitecoinXMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("litecoinx-miner");
    ...
    try { loop {
        ...
        printf("Running LitecoinXMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("LitecoinXMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&LitecoinXMiner, pwallet));
}
